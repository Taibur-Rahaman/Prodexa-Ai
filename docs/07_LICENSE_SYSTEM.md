...
Durable tables live in PostgreSQL (`tenants`, `plans`, `licenses`, `site_activations`, `usage_counters`, `request_nonces`, `normalized_offers`, `discovery_selections`). `POST /v1/license/validate` uses HMAC site authentication (DEC-018). `POST /v1/license/activate` and `POST /v1/license/deactivate` bind or unbind an authenticated site to its server-resolved license (DEC-027): activate is idempotent for the same tenant+site+license, uses `licenses.activation_limit`, and never trusts client `tenant_id`; deactivate sets `site_activations.status` to `revoked` without deleting the site, license, or usage history. `POST /v1/discovery/search` and `POST /v1/discovery/select` reuse that auth and require feature `discovery.search`. After auth, activation counts and usage snapshots may be cached in Redis for 60 seconds keyed by tenant, site, license, and plan version; license status is still loaded from PostgreSQL so revoke/expire is not delayed by the cache. Redis is optional. Staging hosts are not auto-trusted — they must be activated as their own domain. Bootstrap that mints a new site secret from a license key alone is not implemented; activate/deactivate require a pre-provisioned `site_activations` row.
